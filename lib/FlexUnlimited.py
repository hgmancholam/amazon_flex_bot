...
class FlexUnlimited:
    ...
    def __generate_frc(device_id):
        ...
        hmac_ = hmac.new(PBKDF2(device_id, b"HmacSHA256").read(
            32), iv + ciphertext, hashlib.sha256).digest()
  ...
  """
    ...
    def __getTwoStepVerificationChallengeUrl(self, challengeRequest: Response) -> str:
        verificationChallengeCode: str = challengeRequest.get("response").get(
            "challenge").get("uri").split("?")[1].split("=")[1]
    ...
    def getAllServiceAreas(self):
        ...
        serviceAreasTable.field_names = [
            "Service Area Name", "Service Area ID"]
    ...
    def run(self):
        ...
        while self.__retryCount < self.retryLimit:
            ...
            print('\n', self.__retryCount, 'requests attempted')
            ...
            if offersResponse.status_code == 200:
                ...
                Log.info("Rate limit reached. Waiting for " +
                         str(minutes_to_wait) + " minutes.")
