...
class Log:
    ...
    def offer(jsonObj: str, wareHouses):
        ...
        for offer in offers:
            ...
            offerDate = time.strftime(
                '%a %b %#d', time.localtime(offer.get('startTime')))
            offerStartTime = time.strftime(
                '%#I:%M %p', time.localtime(offer.get('startTime')))
            offerEndTime = time.strftime(
                '%#I:%M %p', time.localtime(offer.get('endTime')))
            offerTotalHour = (
                (offer.get('endTime') - offer.get('startTime')) / 3600)
            ...
            msg = "\n--------------------\n"+str(offerDate)+"\n"+str(serviceAreaName)+"\n"+str(offerStartTime) + " - " + str(
                offerEndTime) + " " + str(offerTotalHour)+"hrs\n"+str(priceAmount)+" + " + str(projectedTips)+" = "+str(totalOfferCost)
            return msg
